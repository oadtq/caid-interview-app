...
import { Play, Clock, FileText, ArrowLeft, Brain } from 'lucide-react'
...
export function InterviewCurriculum() {
  ...
  const handleStartLesson = (lesson: any, videoIndex: number = 0) => {
    setSelectedLesson(lesson)
    setSelectedVideoIndex(videoIndex)
  }

  const LessonItem = ({ lesson, type, lessonData, videoIndex }: { 
    lesson: any, 
    type: 'video' | 'article',
    lessonData?: any,
    videoIndex?: number
    ...
    <div 
      ...
      onClick={() => {
        if (type === 'video' && lessonData && videoIndex !== undefined) {
          handleStartLesson(lessonData, videoIndex)
    ...
    >
      <div 
        className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center transition-colors"
      >
        {type === 'video' ? (
